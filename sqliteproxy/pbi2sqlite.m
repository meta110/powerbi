let
    // Новая версия файла находится здесь
...
cache = ( 
    ...
    config = [
        ...
        key_columns = Value.ToText( KeyColumns ) /*Text.Combine( List.Transform( KeyColumns, 
            each Text.Format( "'#{0}'", { Text.From(_) } ) ), "," )*/,
    ...
    // Запуск Python
    #"Run Python script" = Python.Execute(PyScript,[df1=if Data = null then EMPTY_TABLE else data, schema=if Data = null then EMPTY_TABLE else schema, config=Record.ToTable(config)]),
    ...
    transformations = Table.ToRows(#"Removed Columns"),
    ...
    in 
        ...
        else if not List.ContainsAll( Table.ColumnNames( Data ), KeyColumns ) 
