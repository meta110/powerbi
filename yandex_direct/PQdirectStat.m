 работа по API v5 с сервисом Reports httpsyandex.rudevdirectdocreportsreports-docpage
 по-умолчанию работает с отчетом CUSTOM_REPORT. Подробнее про типы отчетов httpsyandex.rudevdirectdocreportstype-docpage
...
let
    stat = (
        ...
        optional tokenYandexMetrika as text  токен доступа к Яндекс можно получить по ссылке httpsoauth.yandex.ruauthorizeresponse_type=token&client_id=764f4af41256427ba87965a7ed31ea3d
    ) =
...
let
    ...
    let
        Source = Web.Contents(
            httpsapi.direct.yandex.comjsonv5, [
    ...
    fnType = type function(
        ...
        optional fields as (type any
            meta [
                ...
                Documentation.FieldDescription = Выберите допустимые поля для отчета CUSTOM_REPORT httpsyandex.rudevdirectdocreportsfields-list-docpage #(cr)Можно передавать в виде списка или в строку с разделителями, например, через запятую
            ]
        ),
        ...
        optional token as (type text 
            meta [
                ...
                Documentation.FieldDescription = Получите токен по ссылке httpsoauth.yandex.ruauthorizeresponse_type=token&client_id=764f4af41256427ba87965a7ed31ea3d,
